...
| --- | --- | --- |
| Light-pollution overlay | [David Lorenz's atlas](https://djlorenz.github.io/astronomy/lp/), currently the 2025 tiles. A separate license for the rendered tiles was not found; see the open question below. | Author and atlas links on the map and below it. Reduced opacity is disclosed. Colors describe simulated zenith brightness, not Bortle classes. The per-point readings (`SKY`, `OVERLOOK_SKY`) sample the same atlas at each of the 40 curated spots and the 119 parkway overlooks. |
...
| Imagery basemap and optional orthophoto | [USGSImageryOnly service metadata](https://basemap.nationalmap.gov/arcgis/rest/services/USGSImageryOnly/MapServer), which credits USDA and USGS The National Map: Orthoimagery. | Both agencies credited and linked. The live tile layer stays online-only. A bounded western North Carolina export is available as an explicit device download. Expanded credits include USGS's [requested acknowledgment](https://www.usgs.gov/faqs/what-are-terms-uselicensing-map-services-and-data-national-map). The service mentions restrictions on Alaska SPOT imagery; do not assume every image served worldwide is public domain. This project's area is western North Carolina. |
| Optional terrain overview | [USGS 3DEP Elevation ImageServer](https://elevation.nationalmap.gov/arcgis/rest/services/3DEPElevation/ImageServer), using its `Hillshade Multidirectional` raster function. | A bounded western North Carolina JPEG is packaged with the orthophoto as an explicit 6.2 MB device download. The exact extent, dimensions, byte sizes, build date, and export requests are retained in `assets/offline-map/manifest.json`. |
