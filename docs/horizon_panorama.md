...
| A spot's `deck:` (the tower's coordinate or its height) | `build_horizons.py`, then `build_canopy.py` | Both cache the whole `[lat, lon, m]` as `deck_at` and recompute when any of it changes. `build_horizons.py` recomputes that spot (1 to 4 s, 2026-09-18); `build_canopy.py` refetches its box (about 25 s from the network at 32 workers; from the `H:` store 10 to 14 s with nothing over the network, measured on the 2026-09-18 16:00 run as Jackrabbit Mountain 10.0 s and Gorges State Park 13.6 s, band pass included, and 10.9 s Fryingpan, 12.4 s Mount Sterling on the 16:33 deck run). Run the terrain first: the block's structures rule compares against the deck terrain line |
...
| `DECK_MIN_R` | 6 m | From the deck, the tower's own cab and roof read as a wall without it. Checked against the lidar 2026-09-18: 90 per cent of the cab-band returns sit within 4.2 m of the Fryingpan deck coordinate and 2.0 m of Mount Sterling's, so 6 m clears both; with the deck placed on the single highest return instead of the band's centre, Fryingpan's cab edge sat 6 to 7 m out and drew spikes to 19 degrees on azimuths 60 to 150. Still a placeholder for any third tower |
